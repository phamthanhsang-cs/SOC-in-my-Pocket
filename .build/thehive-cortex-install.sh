...
OSRPM="fedora35 fedora37 rhel8.5 rhel9.3"
...
EOF
  ...
  then
    wget -qO- https://raw.githubusercontent.com/StrangeBeeCorp/Security/main/PGP%20keys/packages.key |  sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/strangebee-keyring.gpg
    echo 'deb [arch=all signed-by=/etc/apt/trusted.gpg.d/strangebee-keyring.gpg] https://deb.strangebee.com thehive-5.3 main' |  sudo tee -a /etc/apt/sources.list.d/strangebee.list  
  fi
  if [ $? -ne 0 ]
  then
    display_info
...
EOF
  ...
  then
    wget -qO- https://raw.githubusercontent.com/StrangeBeeCorp/Security/main/PGP%20keys/packages.key |  sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/thehive-project.gpg
